...
Note: adding or removing a wordlist requires that `cargo xtask egg` be re-run

1. A list of all the supported wordlists is generated by `cargo xtask egg` (this step is manual and be run when adding or removing a word list!). Their names are generated based on their path - `data/diffenator/latin.txt` becomes `DIFFENATOR_LATIN` in the end crate
2. The build script uses the code generated by the xtask (called `chicken.rs`) to construct URLs to download files
