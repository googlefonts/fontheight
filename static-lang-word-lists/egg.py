...
DATA_DIR_DIFFENATOR = DATA_DIR / "diffenator" 
...
"""
LINE_TEMPLATE = '    (r"{name}", r"{metadata}", r"{path}"),'
...
for text_file in sorted(DATA_DIR_DIFFENATOR.rglob("*.txt")):
    ...
    try:
        name = tomllib.loads((DATA_DIR / metadata_file).read_text(encoding="utf-8"))[
            "name"
        ]
    except KeyError:
        raise KeyError(f"{metadata_file} does not specify `name`, which is required")
